...
`MeshV1ResponderAdmissionCoordinator` is the bounded responder-side handshake/admission transaction. Its record count is exactly the active inbound-authentication bound. It verifies the signed InitiatorHello before deriving any session, emits a signed and responder-confirmed hello, and accepts admission only after the InitiatorFinish confirms the complete transcript. The derived provider session remains staged until admission policy and membership/session capacity preflight succeed. Deferral, rejection, timeout and promotion failure release it before changing pre-authentication state; successful promotion transfers it into `MeshSecuritySessionTable` and may return the exact authenticated direct-peer binding. The whole operation belongs to the serialized Mesh mutation domain, so no intervening membership/session mutation may occur between its preflight and commit. Composition supplies it to `MeshRuntimeResetCoordinator` through the optional pending-authentication reset boundary; staged secrets are released before the provider-wide reset, after which any record whose individual release failed is unconditionally forgotten because the provider has erased all secret slots.
