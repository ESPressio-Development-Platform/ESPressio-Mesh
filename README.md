...
`MeshV1RelayCoordinator` authenticates and removes only the incoming Hop layer, retains the unchanged opaque EndToEnd frame in an explicitly sized fixed slot before accepting responsibility, and rewraps it for the next authenticated hop. A relay retains that frame across downstream Radio failure and pending acceptance; exact authenticated next-hop acceptance, deadline expiry or controlled reset alone releases it. It never receives EndToEnd plaintext.
...
Mesh v1 control acknowledgements use a canonical fixed 56-byte payload. Next-hop responsibility acceptance is direct-hop and Hop protected for the exact previous neighbour. Final destination-framework acknowledgement is a distinct Mesh Control frame protected EndToEnd for the original source and Hop protected along its current route; relays retain and forward it opaquely like any other EndToEnd frame. Both bind the original source identity/incarnation, application `MeshMessageId` and immutable deadline. Authenticated duplicate delivery re-emits acknowledgement intent so a lost final ACK does not require unbounded destination retry state. Neither control message can be fabricated from Radio completion or primitive-operation completion.
