...
public:
    ...
    bool Hash(const std::uint8_t* bytes,std::size_t size,Mesh::MeshSecurityDigest& digest) noexcept override {
        ...
        for(std::size_t i=0;i<size;++i)digest.Value[i%digest.Value.size()]^=static_cast<std::uint8_t>(bytes[i]+i);
...
public:
    Primitive::PrimitiveAdmissionDisposition Next{Primitive::PrimitiveAdmissionDisposition::Accepted};
    std::size_t Calls{0};
...
struct CaptureRadio final {
    ...
    static Mesh::MeshRadioSubmissionResult Submit(void* context,Radio::RadioPeerHandle peer,
        ...
        std::memcpy(self.Packet.data(),bytes,byteCount);return {true,static_cast<std::uint64_t>(70+self.Calls)};
    }
...
int main(){
    ...
    const auto mesh=MeshId(9);
    const auto deviceA=Device(1),deviceB=Device(2),deviceC=Device(3);
    const auto incA=Incarnation(1),incB=Incarnation(2),incC=Incarnation(3);
    const Radio::RadioPeerHandle peerAB{1,1},peerBC{2,1};

    Provider crypto;
    const auto providerAB_A=crypto.CreateSession(1);
    const auto providerAB_B=crypto.CreateSession(2);
    const auto providerBC_B=crypto.CreateSession(3);
    const auto providerBC_C=crypto.CreateSession(4);

    Mesh::AuthenticatedMembershipTable<Memberships> membersA,membersB,membersC;
    ...
    assert(membersB.UpsertAuthenticated(deviceA,incA,Mesh::MembershipState::Active,Mesh::ReachabilityState::Reachable)==
           Mesh::AuthenticatedMembershipInsertResult::Inserted);
    assert(membersB.UpsertAuthenticated(deviceC,incC,Mesh::MembershipState::Active,Mesh::ReachabilityState::Reachable)==
           Mesh::AuthenticatedMembershipInsertResult::Inserted);
    ...
    assert(bindingsB.Bind({deviceC,incC,1,peerBC})==Mesh::DirectPeerBindingResult::Bound);

    Mesh::MeshSecuritySessionTable<Sessions> sessionsA,sessionsB,sessionsC;
    Mesh::MeshSecuritySessionRecordHandle handle{};
    assert(sessionsA.Install(deviceB,incB,SessionId(7),providerAB_A,crypto,handle));
    assert(sessionsB.Install(deviceA,incA,SessionId(7),providerAB_B,crypto,handle));
    assert(sessionsB.Install(deviceC,incC,SessionId(8),providerBC_B,crypto,handle));
    assert(sessionsC.Install(deviceB,incB,SessionId(8),providerBC_C,crypto,handle));

    Mesh::PrimitiveReceiverRegistry<Mesh::Limits::MaxPrimitiveReceivers> receiversA,receiversB,receiversC;
    Receiver receiverB,receiverC;
    receiverB.Next=Primitive::PrimitiveAdmissionDisposition::TemporarilyUnavailable;
    ...
    receiverHandle={};
    assert(receiversC.Register({Primitive::FamilyIds::Event,{1,1},{},Mesh::PrimitiveReceiverExposure::Advertised},
        receiverC,receiverHandle)==Mesh::PrimitiveReceiverRegistrationResult::Registered);

    CaptureRadio radioA,radioB,radioC;
    Mesh::MeshV1FrameWorkspace<512,512> workspaceA,workspaceB,workspaceC;
    Mesh::MeshMessageIdGenerator idsA,idsB,idsC;
    ...
    Mesh::MeshV1BroadcastCoordinator<512,512,4,Memberships,Bindings,Sessions> coordinatorC(
        membersC,bindingsC,sessionsC,crypto,receiversC,radioC.Target(),workspaceC,idsC,mesh,deviceC,incC);

    Mesh::MeshBroadcastFanoutPlan<4> planA,planB,emptyPlan;
    assert(planA.TryAdd({deviceB,incB,1,peerAB}));
    assert(planB.TryAdd({deviceC,incC,1,peerBC}));
    ...
    const auto submitted=coordinatorA.Submit(policy,{Primitive::FamilyIds::Event,1},payload,
        1'000'000'000ULL,250,3,planA);
    assert(submitted.Disposition==Mesh::MeshV1BroadcastDisposition::Completed);
    assert(radioA.Calls==1&&receiverB.Calls==0&&receiverC.Calls==0);

    const auto atB=coordinatorB.Receive(radioA.Packet.data(),radioA.PacketBytes,1'010'000'000ULL,10,planB);
    assert(atB.Disposition==Mesh::MeshV1BroadcastDisposition::DeferredLocal);
    assert(atB.Network==Mesh::MeshBroadcastNetworkDisposition::Forwarded);
    assert(atB.Admission==Primitive::PrimitiveAdmissionDisposition::TemporarilyUnavailable);
    assert(receiverB.Calls==1);
    assert(radioB.Calls==1&&radioB.Peer==peerBC);

    const auto atC=coordinatorC.Receive(radioB.Packet.data(),radioB.PacketBytes,1'020'000'000ULL,20,emptyPlan);
    assert(atC.Disposition==Mesh::MeshV1BroadcastDisposition::Completed);
    assert(atC.Admission==Primitive::PrimitiveAdmissionDisposition::Accepted);
    assert(receiverC.Calls==1&&radioC.Calls==0);

    // Replaying the original authenticated A->B hop with a fresh hop sequence remains
    // the same origin MessageId. B must recognize it as duplicate and must not fan it to C again.
    ...
    assert(crypto.Seal(providerAB_A,Mesh::MeshSecurityTrafficPurpose::Hop,2,duplicatePacket.data(),
    ...
    const auto duplicate=coordinatorB.Receive(duplicatePacket.data(),radioA.PacketBytes,1'030'000'000ULL,30,planB);
    assert(duplicate.Disposition==Mesh::MeshV1BroadcastDisposition::Duplicate);
    assert(receiverB.Calls==1&&radioB.Calls==1);

    // B's delayed local admission retries only locally. It cannot create a second B->C transmission.
    receiverB.Next=Primitive::PrimitiveAdmissionDisposition::Accepted;
    Primitive::PrimitiveAdmissionDisposition retryDisposition{};
    assert(coordinatorB.ServiceDeferredLocal(31,1'040'000'000ULL,retryDisposition)==
           Mesh::MeshDeferredLocalServiceResult::Admitted);
    assert(retryDisposition==Primitive::PrimitiveAdmissionDisposition::Accepted);
    assert(receiverB.Calls==2&&radioB.Calls==1);
    assert(coordinatorB.ServiceDeferredLocal(32,1'050'000'000ULL,retryDisposition)==
           Mesh::MeshDeferredLocalServiceResult::NoWork);
