...
class Provider final : public Mesh::IMeshV1CryptographicProvider {
    ...
    static Mesh::MeshAuthenticationTag Tag(
        Mesh::MeshSecurityTrafficPurpose purpose,std::uint64_t sequence,
        const std::uint8_t* aad,std::size_t aadBytes,
        const std::uint8_t* ciphertext,std::size_t ciphertextBytes) noexcept {
...
public:
    Mesh::MeshSecuritySessionHandle CreateSession(std::size_t slot) noexcept {
        assert(slot>0&&slot<_sessions.size()&&!_sessions[slot]);
        _sessions[slot]=true;
    ...
    bool Hash(const std::uint8_t* bytes,std::size_t size,Mesh::MeshSecurityDigest& digest) noexcept override {
        ...
        for(std::size_t i=0;i<size;++i)
            digest.Value[i%digest.Value.size()]^=static_cast<std::uint8_t>(bytes[i]+i);
...
int main(){
    ...
    Mesh::MeshV1BroadcastHopHeader firstHop{};
    Mesh::MeshV1BroadcastHopView firstView{};
    assert(Mesh::MeshV1BroadcastFrameCodec::DecodeHop(radioA.Packet.data(),radioA.PacketBytes,firstHop,firstView));
    Mesh::MeshV1BroadcastHopHeader duplicateHop=firstHop;
    duplicateHop.Sequence=2;
