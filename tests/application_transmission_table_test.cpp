...
int main() {
    ...
    assert(table.Begin(recipients, 3, payload, 100, 1000, first) == ApplicationTransmissionBeginResult::Begun);
    ...
    assert(table.Begin(duplicateDevice, 2, payload, 100, 1000, invalid) == ApplicationTransmissionBeginResult::DuplicateRecipient);
    ApplicationTransmissionRecipient duplicateMessage[] = {{Device(4), Incarnation(14), 201},{Device(5), Incarnation(15), 201}};
    assert(table.Begin(duplicateMessage, 2, payload, 100, 1000, invalid) == ApplicationTransmissionBeginResult::DuplicateMessageId);
    assert(table.Begin(recipients, 3, {}, 100, 1000, invalid) == ApplicationTransmissionBeginResult::Invalid);
    ...
    assert(table.Begin(recipients, 1, repeatable, 100, 1000, second) == ApplicationTransmissionBeginResult::Begun);
    ...
    assert(table.Begin(recipients + 1, 1, payload, 100, 1000, exhausted) == ApplicationTransmissionBeginResult::ResourceUnavailable);
    assert(table.Release(first));
    ApplicationTransmissionHandle replacement;
    assert(table.Begin(recipients + 1, 1, payload, 100, 1000, replacement) == ApplicationTransmissionBeginResult::Begun);
