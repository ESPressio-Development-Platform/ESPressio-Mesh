...
int main() {
    ...
    const auto firstPeer = Peer(0, 1);
    assert(candidates.Observe(1, firstPeer,
               Mesh::UntrustedMembershipClaim{claimedDevice, claimedIncarnation}, 100, first) ==
    ...
    assert(promotion.CompleteAuthenticated(first, authenticatedDevice, authenticatedIncarnation, &established) ==
           Mesh::AdmissionPromotionResult::PromotedToValidating);
    assert(established.IsValid());
    assert(established.Neighbour == authenticatedDevice);
    assert(established.Incarnation == authenticatedIncarnation);
    assert(established.LocalRadio == 1U);
    assert(established.Peer == firstPeer);
    assert(established.Neighbour != claimedDevice);
    ...
    assert(candidates.Observe(1, Peer(1, 1),
               Mesh::UntrustedMembershipClaim{authenticatedDevice, Incarnation(10)}, 200, conflict) ==
           Mesh::PendingCandidateInsertResult::Inserted);
    ...
    Mesh::AuthenticatedDirectPeerBinding conflictingBinding{};
    assert(promotion.CompleteAuthenticated(conflict, authenticatedDevice, Incarnation(10), &conflictingBinding) ==
           Mesh::AdmissionPromotionResult::ConflictingIncarnation);
    assert(!conflictingBinding.IsValid());
    ...
    assert(candidates.Observe(2, Peer(2, 1),
               Mesh::UntrustedMembershipClaim{Device(2), Incarnation(2)}, 300, rejected) ==
           Mesh::PendingCandidateInsertResult::Inserted);
