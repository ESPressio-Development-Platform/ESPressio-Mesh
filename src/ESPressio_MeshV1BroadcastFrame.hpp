...
struct MeshV1BroadcastHopHeader final {
    ...
    constexpr bool IsValid() const noexcept {
        return static_cast<bool>(Mesh) && static_cast<bool>(Session) && Sequence != 0U &&
               ...
               MessageId != 0U && RemainingResidenceMilliseconds != 0U &&
               HopLimit != 0U && InnerFrameBytes != 0U;
    }
};
...
class MeshV1BroadcastFrameCodec final {
    ...
    static void WriteU16(std::uint8_t* output,std::uint16_t value) noexcept {
        output[0]=static_cast<std::uint8_t>(value>>8U);output[1]=static_cast<std::uint8_t>(value);
    }
    ...
    static void WriteU64(std::uint8_t* output,std::uint64_t value) noexcept {
        for(std::size_t index=0;index<8U;++index) output[index]=static_cast<std::uint8_t>(value>>((7U-index)*8U));
    }
    ...
    static std::uint64_t ReadU64(const std::uint8_t* input) noexcept {
        std::uint64_t value=0;for(std::size_t index=0;index<8U;++index)value=(value<<8U)|input[index];return value;
    }
    static void Copy(std::uint8_t*& output,const std::uint8_t* input,std::size_t size) noexcept {
        std::memcpy(output,input,size);output+=size;
    }
    static void Read(const std::uint8_t*& input,std::uint8_t* output,std::size_t size) noexcept {
        std::memcpy(output,input,size);input+=size;
    }
    static bool EncodeCommon(MeshV1SecurityMessageType type,std::size_t bodyBytes,std::uint8_t* output,std::size_t outputBytes) noexcept {
        if(output==nullptr||outputBytes<CommonHeaderBytes||bodyBytes>std::numeric_limits<std::uint16_t>::max())return false;
        std::memcpy(output,Magic.data(),Magic.size());output[4]=Version;output[5]=static_cast<std::uint8_t>(type);
        WriteU16(output+6U,MeshV1SecuritySuite::Identifier);WriteU16(output+8U,static_cast<std::uint16_t>(bodyBytes));return true;
    }
    static bool DecodeCommon(MeshV1SecurityMessageType type,const std::uint8_t* input,std::size_t inputBytes,std::size_t minimumBodyBytes) noexcept {
        return input!=nullptr&&inputBytes>=CommonHeaderBytes+minimumBodyBytes&&
               ...
               std::memcmp(input,Magic.data(),Magic.size())==0&&input[4]==Version&&input[5]==static_cast<std::uint8_t>(type)&&
               ReadU16(input+6U)==MeshV1SecuritySuite::Identifier&&ReadU16(input+8U)==inputBytes-CommonHeaderBytes;
    }
public:
    ...
    static constexpr std::size_t OriginPacketBytes(std::size_t payloadBytes) noexcept {
        ...
        return payloadBytes!=0U&&body<=std::numeric_limits<std::uint16_t>::max()?CommonHeaderBytes+body:0U;
    }
    ...
    static bool EncodeOriginAuthenticatedHeader(const MeshV1BroadcastOriginHeader& header,std::uint8_t* output,std::size_t outputBytes) noexcept {
        ...
        if(!header.IsValid()||packetBytes==0U||outputBytes<OriginAuthenticatedHeaderBytes||
           !EncodeCommon(MeshV1SecurityMessageType::BroadcastOriginFrame,packetBytes-CommonHeaderBytes,output,outputBytes))return false;
    ...
    static bool DecodeOrigin(const std::uint8_t* input,std::size_t inputBytes,MeshV1BroadcastOriginHeader& header,MeshV1BroadcastOriginView& view) noexcept {
        header={};view={};
        ...
        const auto* cursor=input+CommonHeaderBytes;MeshIdentifier::Storage mesh{};System::DeviceIdentifier::Storage source{};
        ...
        view.SignedBytes=input;view.SignedByteCount=OriginAuthenticatedHeaderBytes+header.PayloadBytes;
        view.Payload=cursor;view.PayloadByteCount=header.PayloadBytes;
    ...
    static bool EncodeHopAuthenticatedHeader(const MeshV1BroadcastHopHeader& header,std::uint8_t* output,std::size_t outputBytes) noexcept {
        ...
        if(!header.IsValid()||packetBytes==0U||outputBytes<HopAuthenticatedHeaderBytes||
           !EncodeCommon(MeshV1SecurityMessageType::BroadcastHopFrame,packetBytes-CommonHeaderBytes,output,outputBytes))return false;
        auto* cursor=output+CommonHeaderBytes;
        Copy(cursor,header.Mesh.Bytes().data(),header.Mesh.Bytes().size());Copy(cursor,header.Session.Value.data(),header.Session.Value.size());
        WriteU64(cursor,header.Sequence);cursor+=8U;
        Copy(cursor,header.Sender.Bytes().data(),header.Sender.Bytes().size());Copy(cursor,header.SenderIncarnation.Bytes().data(),header.SenderIncarnation.Bytes().size());
        Copy(cursor,header.NextHop.Bytes().data(),header.NextHop.Bytes().size());Copy(cursor,header.NextHopIncarnation.Bytes().data(),header.NextHopIncarnation.Bytes().size());
        Copy(cursor,header.Source.Bytes().data(),header.Source.Bytes().size());Copy(cursor,header.SourceIncarnation.Bytes().data(),header.SourceIncarnation.Bytes().size());
        WriteU64(cursor,header.MessageId);cursor+=8U;WriteU32(cursor,header.RemainingResidenceMilliseconds);cursor+=4U;
        *cursor++=header.HopLimit;WriteU16(cursor,header.InnerFrameBytes);return true;
    }

    static bool DecodeHop(const std::uint8_t* input,std::size_t inputBytes,MeshV1BroadcastHopHeader& header,MeshV1BroadcastHopView& view) noexcept {
        header={};view={};
        ...
        const auto ciphertextBytes=inputBytes-HopAuthenticatedHeaderBytes-MeshV1SecuritySuite::AuthenticationTagBytes;
        if(ciphertextBytes==0U||ciphertextBytes>std::numeric_limits<std::uint16_t>::max())return false;
        const auto* cursor=input+CommonHeaderBytes;MeshIdentifier::Storage mesh{};System::DeviceIdentifier::Storage sender{};
        MembershipIncarnation::Storage senderIncarnation{};System::DeviceIdentifier::Storage nextHop{};MembershipIncarnation::Storage nextHopIncarnation{};
        System::DeviceIdentifier::Storage source{};MembershipIncarnation::Storage sourceIncarnation{};
        Read(cursor,mesh.data(),mesh.size());header.Mesh=MeshIdentifier{mesh};Read(cursor,header.Session.Value.data(),header.Session.Value.size());
        header.Sequence=ReadU64(cursor);cursor+=8U;Read(cursor,sender.data(),sender.size());header.Sender=System::DeviceIdentifier{sender};
        ...
        Read(cursor,sourceIncarnation.data(),sourceIncarnation.size());header.SourceIncarnation=MembershipIncarnation{sourceIncarnation};
        header.MessageId=ReadU64(cursor);cursor+=8U;header.RemainingResidenceMilliseconds=ReadU32(cursor);cursor+=4U;
        header.HopLimit=*cursor++;header.InnerFrameBytes=ReadU16(cursor);cursor+=2U;
        if(!header.IsValid()||header.InnerFrameBytes!=ciphertextBytes)return false;
        view.AuthenticatedHeader=input;view.AuthenticatedHeaderBytes=HopAuthenticatedHeaderBytes;view.Ciphertext=cursor;view.CiphertextBytes=ciphertextBytes;
        std::memcpy(view.Tag.Value.data(),cursor+ciphertextBytes,view.Tag.Value.size());return true;
    }
};
