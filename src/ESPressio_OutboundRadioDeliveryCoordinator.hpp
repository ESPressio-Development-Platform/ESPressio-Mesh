...
/// <summary>
/// Owns the bounded Radio-terminal correlation handle for one active outbound delivery while delegating route-attempt,
/// authenticated next-hop acceptance and destination-acknowledgement semantics to their existing coordinators.
/// </summary>
/// <remarks>
/// Managed-Radio scheduler acceptance is never promoted into physical completion or Mesh acceptance. Terminal physical
/// evidence arrives later through the final domain-qualified Radio runtime sink. The neutral relay service class is
/// supplied by the frozen adapter policy and remains unchanged for each submission attempt.
/// </remarks>
...
public:
    OutboundRadioDeliveryCoordinator(
        ...
        ForwardingRadioAttemptCoordinator<CorrelationCapacity,MembershipCapacity,BindingCapacity,HopCapacity>& radioAttempts) noexcept
        :_delivery(delivery),_radioAttempts(radioAttempts) {}
    ...
    OutboundRadioForwardingResult Submit(
        ...
        if(!_delivery.IsActive()||_delivery.AwaitingNextHopAcceptance()||_correlation||!IsMeshRelayServiceClass(service)) return result;
        const auto coordinated=_radioAttempts.Submit(
            localDevice,route,remainingHopLimit,service,payload,payloadSize,nowMilliseconds,_delivery.AbsoluteDeadlineMilliseconds());
        ...
        if(coordinated.Action!=ForwardingAttemptAction::AwaitingNextHopAcceptance){ReleaseCorrelation();return result;}
        if(!coordinated.Submission.NextHop||!coordinated.Submission.NextHopIncarnation){
            ReleaseCorrelation();result.Action=OutboundForwardingAction::Invalid;return result;
        }
        result.Action=_delivery.ArmAcceptedSubmission(
            coordinated.Submission.NextHop,coordinated.Submission.NextHopIncarnation,nowMilliseconds);
    ...
    bool TryConsumeRadioTerminal(
        ...
        if(!_radioAttempts.TryConsumeTerminal(
                handle,nowMilliseconds,_delivery.AbsoluteDeadlineMilliseconds(),forwardingAction,terminal)) return false;
        _correlation={};
    ...
    ForwardingAcceptanceAction AcceptNextHopAuthenticated(
        ...
        std::uint64_t nowMilliseconds,
        RemainingHopLimit& remainingHopLimit) noexcept {
        const auto action=_delivery.AcceptNextHopAuthenticated(
            authenticatedSource,authenticatedSourceIncarnation,acceptedMessageId,nowMilliseconds,remainingHopLimit);
        if(action!=ForwardingAcceptanceAction::IgnoreUnrelatedEvidence)ReleaseCorrelation();
        return action;
    }

    void Reset() noexcept {ReleaseCorrelation();_delivery.Reset();}
};
