...
/// <summary>Fixed-capacity disposable cache of locally planned Mesh routes.</summary>
/// <remarks>
/// Cache contents are never authoritative. A hit is only a planning hint and callers must revalidate it against current
/// membership, topology freshness, link usability and routing policy before forwarding. Replacement is deterministic:
...
public:
    ...
    const Route* Find(const System::DeviceIdentifier& source, const System::DeviceIdentifier& destination) const noexcept {
        if (!source || !destination) return nullptr;
        for (const auto& slot : _slots) {
            if (slot.Occupied && slot.Value.Source() == source && slot.Value.Destination() == destination) return &slot.Value;
        }
        return nullptr;
    }
    ...
    RouteCacheStoreResult Store(const Route& route, RouteCacheHandle& handle) noexcept {
        ...
        for (std::size_t i = 0; i < EntryCapacity; ++i) {
            auto& slot = _slots[i];
            if (!slot.Occupied || slot.Value.Source() != route.Source() || slot.Value.Destination() != route.Destination()) continue;
    ...
    /// <summary>Invalidates the exact cached source/destination hint, if present.</summary>
    bool InvalidatePair(const System::DeviceIdentifier& source, const System::DeviceIdentifier& destination) noexcept {
        if (!source || !destination) return false;
        for (auto& slot : _slots) {
            if (!slot.Occupied || slot.Value.Source() != source || slot.Value.Destination() != destination) continue;
            ClearSlot(slot);
            return true;
        }
        return false;
